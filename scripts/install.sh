...
NPM_NAME="npm-name"
NPM_NAME_REPO="${NPM_NAME}/${NPM_NAME}"
NPM_NAME_GITHUB="https://github.com/${NPM_NAME_REPO}"
NPM_NAME_RELEASES="${NPM_NAME_GITHUB}/releases"

usage() {
    require_cmd cat
    this=$1
    cat 1>&2 <<EOF
$this: download binaries for npm-name
USAGE:
...
ARGS:
    <tag>       is a tag from ${NPM_NAME_RELEASES}. If tag is missing, then the latest will be used.
EOF
    exit 2
}
...
main() {
    ...
    _archive_name="${NPM_REPO}-${_arch}${_ext}"

    if [ -z "${TAG}" ]; then
        println "The latest version will be installed."
        _url="${NPM_NAME_RELEASES}/latest/download/${_archive_name}"
    else
        println "Version ${TAG} will be installed"
        _url="${NPM_NAME_RELEASES}/download/${TAG}/${_archive_name}"
    fi
